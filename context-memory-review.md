...
**方向合理，注入层有冗余。** Agenora 的三层记忆栈（短期消息 → 滚动摘要 → 跨会话 `UserMemory`）+ KB RAG 正交通道本身没有问题；精度优先、静默写入、Provider 安全合并也做对了。真正需要收敛的是「Profile + 检索记忆」双通道重叠、预算预留过保守、以及文档落后于实现——不是推倒重来。
...
- [x] 记忆导出 / 逐条过期编辑
- [x] 真实 tokenizer（tiktoken + 启发式兜底）
- [ ] 离线评测小集；记忆量上来再迁专用向量索引

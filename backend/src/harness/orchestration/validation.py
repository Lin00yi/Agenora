"""Validate harness task DAGs and bind them to registered capabilities."""
...
def match_agent(registry: AgentRegistry, *, task_type: str, capabilities: list[str], has_kb: bool) -> str:
    ...
    for agent_id in available:
        provided = set(registry.get(agent_id).provides)
    ...
    if not scored and preferred in available and not needed:
        return preferred
    if not scored:
        raise DagValidationError(f"no agent provides {sorted(needed)} for type={task_type} has_kb={has_kb}")
    scored.sort()
    return scored[0][1]
...
def validate_and_bind(payload: Any, *, registry: AgentRegistry, has_kb: bool) -> TaskDag:
    ...
    tasks = [_normalize_task(item, index=index) for index, item in enumerate(raw_tasks)]
    ...
    for task in tasks:
        for dependency in task.get("depends_on") or []:
            if dependency not in id_set:
                raise DagValidationError(f"unknown depends_on: {dependency}")
    ...
    return {
        "tasks": bound,
        "reason": (str(payload.get("reason") or "planned").strip() or "planned")[:80],
...
def ready_tasks(dag: TaskDag, status: dict[str, str]) -> list[TaskNode]:
    return [
        ...
        if status.get(str(task.get("id") or ""), "pending") == "pending"
        and all(status.get(dependency) == "done" for dependency in (task.get("depends_on") or []))
    ]

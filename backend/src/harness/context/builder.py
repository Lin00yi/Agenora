...
async def build_context_for_conversation(
    ...
    *,
    ...
    with token_model_scope(model):
        ...
        # Keep every turn that has not entered the rolling summary, then trim
        # from the oldest by token budget. A fixed recent-turn cap belongs in
        # the summarizer, not here: otherwise early rows vanish before they
        # are represented by a summary.

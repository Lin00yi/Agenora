...
当前摘要是确定性抽取式摘要：按时间顺序保留用户/助手消息，每条消息最多约 280 字，总长度再受摘要 token 预算限制。它不会调用 LLM，因此不会额外消耗模型费用，也尽量避免编造事实。
...
- 检索以关键词为主，尚未为 Memory 建立向量索引；
- `expires_at` 已预留，但尚未实现自动过期任务；
- `constraint` 使用内容哈希键，语义上相近但措辞不同的规则不会自动合并；
- 尚未提供独立的 Memory 管理页面，仅提供后端 API 与前端调用封装；
- 会话摘要仍是抽取式摘要，后续可替换为结构化 LLM 摘要。

下一阶段建议优先引入：Memory 向量检索、结构化约束键、自动过期策略，以及设置页中的 Memory 管理界面。

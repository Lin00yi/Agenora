...
普通聊天与知识库问答使用唯一的受约束 ReAct Runtime：

```text
规则 → triage → complex → 回退 → scope（ACL KB 范围选择） → reason ⇄ call_tools
```

`scope` 在规则或分层意图识别判定为知识型问题后，才可从当前用户可读的 KB 候选中选择最多三个库；不会为普通对话调用 KB 路由模型，也不会创建任务 DAG 或授予未挂载工具。随后同一个模型在一个循环内决定是否调用 `search_kb`、`search_kg`、`web_search` 等当前已授权工具；调用次数、网页证据数、提示注入限制、工具白名单和输出上下文预算都由服务端强制执行。

订单和退款不由模型授予写入能力：规则识别出的订单意图才进入独立的确定性审批图，退款仍要求下一条用户消息精确确认。历史 Supervisor Trace 保留为审计记录，但该运行时及其 checkpoint 不再恢复。
